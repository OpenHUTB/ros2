...
#
# Adapted for the OpenHUTB simulator (package "hutb", provides the `carla`
# module). Changes over the upstream CARLA example: --vehicle to force a
# blueprint id and --vehicle-pool to cycle vehicles with BACKSPACE.
# Install the matching client before running, see README.md.
...
# ==============================================================================
# -- find carla module ---------------------------------------------------------
# The `carla` module is provided by the "hutb" wheel shipped with the
# OpenHUTB simulator. Try the wheel first; fall back to the .egg in the
# simulator's PythonAPI directory (for in-tree development).
# ==============================================================================
...
class World(object):
    ...
    def restart(self):
        ...
        # 控制台回显当前车型，方便确认「选车器」是否生效。
        try:
            print('    Driving vehicle : %s' % blueprint.id)
            if self._vehicle_pool:
                print('    Vehicle pool    : %s   [按 Backspace 轮换]' % ', '.join(self._vehicle_pool))
            elif self._vehicle_override:
                print('    Vehicle fixed   : BACKSPACE 只做原位重置，不改车型')
        except Exception:
            pass
...
def main():
    ...
    argparser.add_argument(
        ...
        metavar='IDS',
        default='',
        help='comma separated blueprint ids; BACKSPACE cycles through this list')
    argparser.add_argument(
        '--gamma',
        default=2.2,
        type=float,
