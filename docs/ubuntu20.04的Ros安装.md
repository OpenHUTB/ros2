...
| 问题现象 | 解决方法 | 对应章节 |
| :--- | :--- | :--- |
| `NO_PUBKEY` 签名错误 | 用 `apt-key --recv-keys` 重新导入公钥 | 4.1 |
| 找不到 `rosdep` 命令 | `sudo apt install python3-rosdep2` | 7.1 |
| 无法下载 default sources list | 修改 `/etc/hosts` 指定 GitHub 域名 IP | 7.2 |
| default sources list already exists | `sudo rm` 删除残留文件后重新初始化 | 7.3 |
| `roscore` 命令未找到 | `sudo apt install python3-roslaunch` | 10.2 |
| `Resource not found: roslaunch` | 先 `source` 环境变量，必要时重装完整版 | 10.2 |
