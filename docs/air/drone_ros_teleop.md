...
![手柄遥控真机测试演示](../img/air/test_xbox.gif)
...
节点按「飞智冰原狼 4（Xbox 协议）+ joy_node + AirSim 飞控底座」真机 HIL 标定结果，把摇杆
轴值映射为速度指令（轴序与符号均已实测校准，注意 `axes[4]` 是 RT 扳机而非摇杆轴）：
...
* **边界保护**：映射前检查 `len(msg.axes) > max(轴索引)`，轴数组过短（低维手柄）时丢弃本帧并
  告警，防止索引越界导致节点闪退；
* **参数化配置**：轴索引与缩放系数均为 ROS 私有参数（`~axis_*`、`~scale_*`、`~deadzone`），
  默认值即上述标定值，可经 `rosparam set` 或 launch 文件按实际手柄/驱动微调，无需改源码；
* **符号内置于缩放系数**：如左推 `axes[0]` 为 `+1`，但 AirSim 左偏航需负角速度，故
  `scale_yaw = -1.0`；其余符号同理。
